...
commander__WEBPACK_IMPORTED_MODULE_0___default.a.version('0.5.42', '-v, --version').usage('<sub-command>'); // 根据csv文件输出json文件
...
function createPropFile(prop, index, csdPath, tempFile, stocks) {
  ...
  stocks.forEach(function (arr, i) {
    var code = arr[0]; // 通达信自定义数据格式：北证以2标志、上证以1标志、深证以0标志；

    var szh = /^[6]/.test(code) ? 1 : /^[84]/.test(code) ? 2 : 0;
