...
/* harmony default export */ __webpack_exports__["default"] = (function (program) {
  program.command('fetch').description('从网络爬取股票数据, Usage: jhandy fetch -s "ths_new ths_p" -c 300059').option('-s, --sources <source_id>', '要爬取的数据源网页id:ths_new, ths_p, ths_c, ycj, 默认爬取全部数据源').option('-i, --index <index>', '股票列表索引位置,主要用于上次爬取数据中断', function (val) {
    return val * 1;
  }).option('-c, --stocks <stocks>', '*.json 或者 000001,000002').option('-p, --csd-path <csd-path>', 'csd路径').action(function (cmd) {
...
/* harmony default export */ __webpack_exports__["default"] = (function (program) {
  program.command('tdx').description('通达信自定义数据输出, Usage: jhandy tdx -d').option('-c, --csd-path [csd路径]', 'csd数据存储路径').option('-f, --tdx-file [extern_user.txt 路径]', '通达信自定义数据文件路径').option('-p, --prop [prop name]', "prop: ['概念', '概念y', '产品', '业务', '全名', '备注']").option('-d, --use-default', '使用default选项').action(function (cmd) {
